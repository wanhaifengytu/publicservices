...
package com.sap.hadoop.test.spark.org.apache.spark.examples;
  ...
  public static void main(String[] args) {
   /* SparkSession spark = SparkSession
    ...
    JavaSparkContext jsc = new JavaSparkContext(spark.sparkContext()); */
		SparkConf conf = new SparkConf().setMaster("local").setAppName("Log Query");
		// Create a Java version of the Spark Context
		JavaSparkContext sparkContext = new JavaSparkContext(conf);

    JavaRDD<String> dataSet = (args.length == 1) ? sparkContext.textFile(args[0]) : sparkContext.parallelize(exampleApacheLogs);
