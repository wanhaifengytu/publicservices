...
public class SparkProcessLogs {
	...
	public static Dataset<Row> joinTwoDatasets(Dataset<Row> left, Dataset<Row> right, String joinType, String joinFieldLeft, String joinFieldRight) {
		
		Dataset <Row> joinedData = left.join(right, left.col(joinFieldLeft).equalTo(right.col(joinFieldRight)),joinType);
		return joinedData;
	}
	...
	public static Dataset<Row> parseServerLogs(SparkSession session, String serverlogCSVPath) {
		...
		//CommonUtil.printTop5RowsOfDataset("./rowServerLogs.csv", rowServerLogs);
